...
Class Characters {
                    ...
                    case SERVER_TRINITY:
                        for($k = 1; $k < 6; $k++) {
                            if($char_talent['spell'] == $class_talent['Rank_' . $k]) {
                                $talent_build[$char_talent['specMask']] .= $k;
                                $current_found = true;
								$talent_points[$char_talent['specMask']][$class_talent['TalentTab']] += $k;
								//print("talent_build: ".$talent_build[$char_talent['specMask']].", char_talent: ".$char_talent['specMask'].", class_talent: ".$class_talent['TalentTab'].", k: ".$k.", talent_points: ".$talent_points[$char_talent['specMask']][$class_talent['TalentTab']]."\r\n");
                            }
                        }
                        $last_spec = $char_talent['specMask'];
                        break;
                    default:
    ...
    public function CalculateCharacterTalentBuild() {
        ...
        foreach($character_talents as $_tal) {
            ...
            elseif($this->m_server == SERVER_TRINITY) {
                $specs_talents[$_tal['specMask']][$_tal['spell']] = true;
            }
        }
        if($this->m_server == SERVER_TRINITY) {
            for($i = 0; $i < 3; $i++) {
    ...
    public function GetCharacterGlyphs($spec = -1) {
        ...
        switch($this->m_server) {
            case SERVER_MANGOS:
                if($spec >= 0) {
                    $glyphs_data = $this->db->select("SELECT * FROM `character_glyphs` WHERE `guid`=%d AND `talentGroup`=%d ORDER BY `slot`", $this->guid, $spec);
                }
                else {
                    $glyphs_data = $this->db->select("SELECT * FROM `character_glyphs` WHERE `guid`=%d ORDER BY `talentGroup`, `slot`", $this->guid);
                }
                break;
            case SERVER_TRINITY:
                if($spec >= 0) {
                    $glyphs_data = $this->db->select("SELECT * FROM `character_glyphs` WHERE `guid`=%d AND `talentGroup`=%d", $this->guid, $spec);
                }
                else {
                    $glyphs_data = $this->db->select("SELECT * FROM `character_glyphs` WHERE `guid`=%d ORDER BY `talentGroup`", $this->guid);
                }
                break;
        }
        if(!$glyphs_data) {
    ...
    public function GetMaxEnergy() {
        ...
        elseif($this->class == CLASS_ROGUE) {
            ...
            switch($this->m_server) {
                case SERVER_MANGOS:
                    $isGlyphed = $this->db->selectCell("SELECT 1 FROM `character_glyphs` WHERE `guid`=%d AND `glyph`=408 AND `spec`=%d", $this->guid, $this->activeTalentGroup);
                    if($isGlyphed) {
                        $maxPower = 120;
                    }
                    break;
                case SERVER_TRINITY:
                    $isGlyphed = $this->db->selectCell("SELECT 1 FROM `character_glyphs` WHERE `guid`=%d AND (`glyph1`=408 OR `glyph2`=408 OR `glyph3`=408 OR `glyph4`=408 OR `glyph5`=408 OR `glyph6`=408) AND `talentGroup`=%d", $this->guid, $this->activeTalentGroup);
                    if($isGlyphed) {
                        $maxPower = 120;
                    }
                    break;
    ...
    public function HasTalent($talent_id, $active_spec = true, $rank = -1) {
        switch($this->GetServerType()) {
            ...
            case SERVER_TRINITY:
                ...
                $sql_data = array(
                    'activeTalentGroup' => array(
                        sprintf('SELECT `spell` FROM `character_talent` WHERE `spell` IN (%s) AND `guid`=%%d AND `specMask`=%%d LIMIT 1', $talent_spells['Rank_1'] . ', ' . $talent_spells['Rank_2'] . ', ' . $talent_spells['Rank_3'] . ', ' . $talent_spells['Rank_4'] . ', ' . $talent_spells['Rank_5']),
                        sprintf('SELECT 1 FROM `character_talent` WHERE `spell`=%d AND `guid`=%%d AND `specMask`=%%d', $rank == -1 ? $talent_spells['Rank_1'] : $talent_spells['Rank_' . ($rank + 1)])
                    ),
                    'specMask' => array(
        ...
        if($activeTalentGroup) {
            if($rank == -1) {
                $has = $this->db->selectCell($sql_data['activeTalentGroup'][0], $this->guid, $this->activeTalentGroup);
            }
            elseif($rank >= 0) {
                $has = $this->db->selectCell($sql_data['activeTalentGroup'][1], $this->guid, $this->activeTalentGroup);
            }
        }
        else {
            if($rank == -1) {
                $has = $this->db->selectCell($sql_data['specMask'][0], $this->guid);
            }
            elseif($rank >= 0) {
                $has = $this->db->selectCell($sql_data['specMask'][1], $this->guid);
            }
        }
